...
- **Admin System Implementation (27 Okt 2025)**: Complete admin panel with authentication, dashboard, IKM management, feedback moderation, gallery & document management. Features: CSRF protection, rate limiting (10 login attempts/hour), password hashing, session management. Access: /admin/login. Admin pertama dibuat via `python create_admin.py` (tidak ada default credentials).
- **Content Folder Organization (27 Okt 2025)**: Moved all content_*.md files to content/ folder for better project structure and maintainability. Updated load_markdown() function to read from content/ directory.
- **Production Deployment Files (27 Okt 2025)**: Created complete VPS deployment documentation and configuration files for Hostinger VPS deployment including wsgi.py, production logging, Nginx config, systemd service, backup script, and comprehensive DEPLOYMENT.md guide
