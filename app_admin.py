...
def dashboard():
    ...
    try:
        cur.execute('SELECT COUNT(*) as total FROM ikm_responses')
        ...
        cur.execute('''
            SELECT 
                AVG((q1 + q2 + q3 + q4 + q5 + q6 + q7 + q8 + q9) / 9.0) * 25 as avg_ikm
            FROM ikm_responses
        ''')
        ...
        cur.execute('''
            SELECT DATE(submitted_at) as tanggal, COUNT(*) as jumlah
            FROM ikm_responses
        ...
        cur.execute('''
            SELECT submitted_at, name, comments
            FROM ikm_responses
        ...
        cur.execute('''
            SELECT submitted_at, name, message
            FROM feedback
            ORDER BY submitted_at DESC
            LIMIT 5
        ''')
...
def ikm_list():
    ...
    try:
        ...
        cur.execute('''
            SELECT 
                id, name, phone, submitted_at,
                (q1 + q2 + q3 + q4 + q5 + q6 + q7 + q8 + q9) / 9.0 * 25 as score,
                comments
            FROM ikm_responses
            ORDER BY submitted_at DESC
            LIMIT %s OFFSET %s
...
def ikm_detail(id):
    ...
    try:
        cur.execute('SELECT * FROM ikm_responses WHERE id = %s', (id,))
